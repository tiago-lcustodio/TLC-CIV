...
class GerenciadorEventos:
    ...
    Sistemas futuros podem registrar callbacks sem criar dependências diretas no
    loop principal. O histórico curto também ajuda em depuração e futuros logs.
    """

    def __init__(self, limite_historico=200):
        self._ouvintes: Dict[
            str,
            List[Callable[[EventoJogo], None]]
        ] = {}
    ...
    def registrar(
        self,
        tipo: str,
        callback: Callable[[EventoJogo], None]
    ):
        self._ouvintes.setdefault(tipo, []).append(callback)

    def remover(self, tipo: str, callback):
        if (
            tipo in self._ouvintes
            and callback in self._ouvintes[tipo]
        ):
